{$page_context = 'cerberusweb.contexts.asset'}
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Edit -->
		<button type="button" id="btnDisplayAssetEdit" title="{'common.edit'|devblocks_translate|capitalize}"><span class="glyphicons glyphicons-cogwheel"></span></button>
	</form>
...
<fieldset class="properties">
	<legend>{'Asset'|devblocks_translate|capitalize}</legend>
...
<script type="text/javascript">
	$(function() {
		...
		tabOptions.active = Devblocks.getjQueryUiTabSelected('profileAssetTabs');
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#profileAssetTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplayAssetEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {
